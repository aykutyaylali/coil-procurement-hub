...
> Son güncelleme: 7. oturum — sipariş PDF logo + tek-tıkla sipariş + award→talep ORDERED +
> bid formu PB sadeleştirme + **kalem bazlı fotoğraf (talep→tedarikçi)** + **kalite bobin testleri paneli**.

## 7. oturum — kullanım geri bildirimleri: sipariş, PDF, foto & kalite

Canlı kullanımda bildirilen sorunlar kök nedeninden çözüldü; her biri ayrı commit:

- **Sipariş PDF logosu (`2802889`)** — PDF'lerde placeholder "C" monogramı yerine gerçek `coil-logo.png`
  (yüklenemezse güvenli düşüş — PDF asla bozulmaz). Aynı commit'te sipariş süreci sadeleştirildi:
  taslak siparişte **tek "Onayla ve Tedarikçiye Gönder" butonu** (`confirmAndSendOrder`; yönetim onayı
  opsiyonel kalır). Ayrıca **award→talep ORDERED** düzeltmesi: RFQ karara bağlanınca bağlı talep/kalemler
  otomatik ORDERED olur (talep artık "Teklifler toplanıyor"da takılı kalmaz). E2E ile doğrulandı.
- **Bid formu PB sadeleştirme (`316ae1a`)** — gereksiz "Varsayılan Para Birimi" alanı kaldırıldı
  (PB zaten kalem bazlı). Navluna kendi PB seçicisi; teklifin ana PB'si kalemlerden türetilir.
- **Kalem bazlı fotoğraf + kalite bobin testleri (`8a20add`, `fa229dd`)** — mevcut polimorfik `Attachment`
  altyapısı UI'ya bağlandı (yeniden kullanılabilir `AttachmentUploader`; `listAttachments`/`deleteAttachment`).
  Talep kalemlerine foto (hem **açılırken** yeni formda hem detayda); `isInternal=false` → **tedarikçinin
  teklif sayfasında** kalem altında salt-okunur önizleme (`bidding.ts › loadLinePhotos`, üç loader'a da
  data-URL gömülür; tedarikçi oturumsuz). Kalite modülünde **CoilTestsPanel**: düzenlenebilir test tablosu
  (test/metot/spec/ölçüm/sonuç) + sık testler hızlı ekleme + test raporu/fotoğraf ekleme (`QualityInspection.testsJson`).
