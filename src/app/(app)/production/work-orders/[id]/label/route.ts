...
import { buildWorkOrderLabelPdf } from "@/lib/pdf/documents";
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const buf = await buildWorkOrderLabelPdf(id, auth.tenantId, auth.locale);
    return pdfResponse(buf, `IsEmri-${id}.pdf`);
  } catch (e) {
