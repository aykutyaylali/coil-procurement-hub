...
export function NewInvoiceForm({ orders, preselectOrderId, lmeRecords = [], defaultUsdTry = "" }: { orders: Order[]; preselectOrderId: string; lmeRecords?: LmeRec[]; defaultUsdTry?: string }) {
  ...
  async function submit() {
    if (!order) return;
    if (!number.trim()) { setError(t("inv.new.numberRequired")); return; }
  ...
  if (orders.length === 0) {
    return <Card><EmptyState title={t("inv.new.empty.title")} hint={t("inv.new.empty.hint")} /></Card>;
  }

  return (
    <div className="space-y-6">
      <Card>
        <CardHeader><CardTitle>{t("inv.new.info")}</CardTitle></CardHeader>
        <CardContent className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="space-y-1.5">
            <Label>{t("inv.new.orderField")}</Label>
          ...
          <div className="space-y-1.5">
            <Label>{t("inv.new.numberField")}</Label>
            <Input value={number} onChange={(e) => setNumber(e.target.value)} placeholder={t("inv.new.numberPlaceholder")} />
          </div>
          <div className="space-y-1.5">
            <Label>{t("inv.new.invoiceDate")}</Label>
            <Input type="date" value={invoiceDate} onChange={(e) => setInvoiceDate(e.target.value)} />
          </div>
          <div className="space-y-1.5">
            <Label>{t("inv.new.dueDate")}</Label>
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">{t("inv.lme.title")}</CardTitle></CardHeader>
        <CardContent className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <div className="space-y-1.5">
            <Label>{t("inv.lme.mode")}</Label>
            <Select value={lmeMode} onChange={(e) => setLmeMode(e.target.value)}>
              <option value="ORDER">{t("inv.lme.modeOrder")}</option>
              <option value="INVOICE_PERIOD">{t("inv.lme.modeInvoicePeriod")}</option>
            </Select>
          </div>
          {lmeMode === "INVOICE_PERIOD" && (
            <>
              <div className="space-y-1.5">
                <Label>{t("inv.lme.record")}</Label>
                <Select value={lmeRecordId} onChange={(e) => setLmeRecordId(e.target.value)}>
                  <option value="">{t("inv.lme.select")}</option>
                  {lmeRecords.map((r) => <option key={r.id} value={r.id}>{new Date(r.priceDate).toLocaleDateString("tr-TR")} · {r.kind === "WEEKLY_AVG" ? t("inv.lme.week") : t("inv.lme.day")} · {Number(r.usdPerTon).toLocaleString("tr-TR")}</option>)}
                </Select>
              </div>
              <div className="space-y-1.5">
                <Label>{t("inv.lme.usdTry")}</Label>
                <Input value={invoiceUsdTry} onChange={(e) => setInvoiceUsdTry(e.target.value)} placeholder={t("inv.lme.usdTryPlaceholder")} />
              </div>
            </>
          )}
          <p className="text-xs text-muted-foreground sm:col-span-2 lg:col-span-3">{t("inv.lme.note")}</p>
        </CardContent>
      </Card>

      <Card>
        <CardHeader><CardTitle>{t("inv.lines.title")}</CardTitle></CardHeader>
        <CardContent className="p-0">
          <Table>
            <THead>
              <TR><TH></TH><TH>{t("inv.match.item")}</TH><TH className="text-right">{t("inv.lines.ordered")}</TH><TH className="text-right">{t("inv.match.received")}</TH><TH className="text-right">{t("inv.lines.invoiceQty")}</TH><TH className="text-right">{t("inv.lines.unitPrice")}</TH><TH className="text-right">{t("inv.lines.vatPct")}</TH><TH className="text-right">{t("inv.net")}</TH></TR>
            </THead>
            <TBody>
              {order?.lines.map((l) => {
                const s = get(l);
      ...
      <Card>
        <CardContent className="flex flex-wrap items-end justify-between gap-4 pt-6">
          <div className="space-y-1.5">
            <Label>{t("inv.withholdingAmount")}</Label>
            <Input value={withholding} onChange={(e) => setWithholding(e.target.value)} className="w-40" />
          </div>
          <div className="text-right text-sm">
            <div>{t("inv.net")}: <b>{formatMoney(net, order?.currency ?? "TRY")}</b></div>
            <div>{t("inv.vat")}: <b>{formatMoney(tax, order?.currency ?? "TRY")}</b></div>
            <div className="text-base">{t("inv.grandTotal")}: <b>{formatMoney(grand, order?.currency ?? "TRY")}</b></div>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="flex justify-end gap-2">
        <Button variant="outline" onClick={() => router.back()} disabled={busy}>{t("inv.cancel")}</Button>
        <Button onClick={submit} disabled={busy}>{busy ? t("inv.saving") : t("inv.saveAndMatch")}</Button>
      </div>
    </div>
  );
}
